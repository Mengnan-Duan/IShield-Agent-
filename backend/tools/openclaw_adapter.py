...
def _agent_runtime_conclusion(decision: str, reason: str, tool_name: str) -> str:
    status_code = _agent_status_code(decision)
    if status_code == "blocked":
        return f"Agent工具调用已阻断：{tool_name}。{reason}".strip()
    if status_code == "confirm":
        return f"Agent工具调用需要确认：{tool_name}。{reason}".strip()
    if status_code == "allowed":
        return f"Agent工具调用已放行：{tool_name}，审计证据已记录。"
    if status_code in {"timeout", "error"}:
        return f"Agent工具调用异常：{tool_name}。{reason}".strip()
    return reason or f"Agent工具调用已完成：{tool_name}"
...
class AgentMonitor:
    ...
    def execute_tool(
        ...
        chain_id = chain_id or f"chain-agent-{call_id}"
        ...
        # ── 步骤5：记录调用历史 ──────────────────────────────
        status_code = _agent_status_code(decision)
        runtime_conclusion = _agent_runtime_conclusion(decision, reason, tool_name)
        try:
            add_event(
                event_type="Agent运行结论",
                detail=runtime_conclusion,
                status="已阻断" if status_code == "blocked" else "需确认" if status_code == "confirm" else "已放行",
                text_hash="",
                threat_level="high" if status_code == "blocked" else "medium" if status_code == "confirm" else "low",
                confidence=confidence,
                source_ip=ip,
                action=tool_name,
                tool_name=tool_name,
                ...
                metadata={
                    ...
                    "tool_evidence": {
                        "tool": tool_name,
                        "target": params_preview,
                        "decision": status_code,
                        "summary": (tool_result or {}).get("summary") or runtime_conclusion,
                        "reason": reason,
                    },
                    "steps": [
                        {
                            ...
                            "detail": policy_reason or attack_reason or "策略检查完成",
                            "risk_score": confidence,
                        },
                        {
                            "stage": "agent_tool_finished",
                            "title": "工具调用结果",
                            "status": status_code,
                            "detail": (tool_result or {}).get("summary") or runtime_conclusion,
                            "risk_score": confidence,
                        },
                    ],
                },
