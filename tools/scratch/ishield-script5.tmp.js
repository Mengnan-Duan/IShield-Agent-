...
function renderLog() {
  ...
  if (filtered.length === 0) {
    log.innerHTML = '<div class="ops-empty-box"><div class="ops-empty-title">??????</div><div class="ops-empty-desc">?????????????? / ?????????????</div></div>';
    return;
  }
  log.innerHTML = '';
  filtered.forEach(function(ev, i) {
    ...
    var statusText = ev.status || (blocked ? '???' : '???');
    ...
    row.innerHTML =
      '<div class="soc-event-head">' +
        '<div><div class="soc-event-title">' + esc(typeText) + '</div><div class="soc-event-detail">' + esc(ev.detail || '???') + '</div></div>' +
...
async async function loadChains() {
  ...
  try {
    ...
    if (!cachedChains.length) {
      el.innerHTML = '<div class="ops-empty-box"><div class="ops-empty-title">?????</div><div class="ops-empty-desc">??????????????????????????????</div></div>';
      return;
    }
    el.innerHTML = cachedChains.map(function(chain){
      var blocked = isBlockedStatus(chain.status);
      var review = /confirm|review|??|??|?/.test(String(chain.status || '').toLowerCase());
      ...
      return '<button type="button" data-chain-id="'+esc(chain.chain_id || '')+'" class="chain-summary-btn soc-chain-card">' +
        '<div class="soc-event-head">' +
          ...
          '<div class="soc-event-detail">?? '+esc(chain.source_ip || '--')+' ? ?? '+esc(chain.tool_name || chain.action || '--')+' ? ?? '+esc(chain.target || '--')+'</div></div>' +
          '<span class="ops-status-pill '+pill+'">'+esc(chain.status || '???')+'</span>' +
        '</div>' +
    ...
    el.innerHTML = '<div class="ops-empty-box"><div class="ops-empty-title">???????</div><div class="ops-empty-desc">'+esc(e.message)+'</div></div>';
  }
}
...
function renderEventDetail(eventData, chain) {
  ...
  var decision = blocked ? '???????' : (isPassedStatus(eventData.status) ? '??????' : '???????');
  var chainHtml = (chain || []).map(function(step){
    ...
    '</div>';
  }).join('') || '<div class="ops-empty-box">?????????????????</div>';
  content.innerHTML =
    '<div class="space-y-5">' +
      '<section class="policy-verdict '+(blocked ? 'blocked' : 'allowed')+'">' +
        '<div><div class="sim-title">'+decision+'</div><div class="sim-sub">?????????????????????????????????</div></div>' +
        '<span class="ops-status-pill '+pill+'">'+esc(eventData.status || '--')+'</span>' +
      '</section>' +
      '<div class="grid grid-cols-1 md:grid-cols-3 gap-4">' +
        forensicMetric('?? ID', eventData.id) +
        forensicMetric('??? ID', eventData.chain_id || '--') +
        forensicMetric('??', eventData.stage || '--') +
        forensicMetric('??', eventData.rule_id || '--') +
        forensicMetric('?? IP', eventData.source_ip || '--') +
        forensicMetric('??', eventData.target || '--') +
      '</div>' +
      '<section class="detect-evidence"><h4 class="detect-section-title">????</h4><div class="soc-event-detail">'+esc(eventData.detail || '')+'</div></section>' +
      '<section class="detect-evidence"><h4 class="detect-section-title">??????</h4><div class="soc-detail-timeline">'+chainHtml+'</div></section>' +
      '<section class="detect-evidence"><h4 class="detect-section-title">?????</h4><pre class="text-xs whitespace-pre-wrap break-all font-mono" style="color:var(--text-secondary);line-height:1.7;">'+esc(JSON.stringify(meta, null, 2))+'</pre></section>' +
    '</div>';
}
...
function renderChainDetail(chainData) {
  ...
  var timeline = events.map(function(step, index){
    ...
    return '<div class="soc-timeline-item">' +
      '<div class="flex items-start justify-between gap-3 flex-wrap"><div><div class="soc-event-title">#'+(index+1)+' '+esc(step.type || '??')+'</div><div class="soc-event-meta mt-1"><span>'+esc(step.time || '--')+'</span><span>'+esc(step.stage || '--')+'</span><span>'+esc(step.rule_id || '--')+'</span></div></div><span class="ops-status-pill '+stepPill+'">'+esc(step.status || '??')+'</span></div>' +
      '<div class="soc-event-detail mt-3">'+esc(step.detail || '')+'</div>' +
      '<details class="mt-3"><summary class="cursor-pointer text-xs font-mono" style="color:var(--accent);">?????</summary><pre class="mt-2 text-xs whitespace-pre-wrap break-all font-mono" style="color:var(--text-secondary);line-height:1.7;">'+esc(JSON.stringify(meta, null, 2))+'</pre></details>' +
    '</div>';
  }).join('') || '<div class="ops-empty-box">????????????</div>';
  ...
  content.innerHTML =
    '<div class="space-y-5">' +
      '<section class="policy-verdict '+(blocked ? 'blocked' : 'allowed')+'">' +
        '<div><div class="sim-title">'+esc(chainData.chain_id || '?????')+'</div><div class="sim-sub">'+esc(chainData.source_ip || '--')+' ? '+esc(chainData.tool_name || chainData.action || '--')+' ? '+esc(chainData.target || '--')+'</div></div>' +
      ...
      '<section class="detect-evidence"><h4 class="detect-section-title">???????</h4><div class="soc-detail-timeline">'+timeline+'</div></section>' +
    '</div>';
}
